%Para rodar essa rotina é necessário o pacote OET do delft baixado a parte do repositório do Deltares.
...
FlowElem_xcc=nc_varget('*_map.nc','FlowElem_xcc'); %posicao em x
FlowElem_ycc=nc_varget('*_map.nc','FlowElem_ycc'); % posicao em y

%Criando xq e yq, valores que eu quero resgatar (Como se fosse OBS points).
m = load('tide_points.txt'); %Points (lat,lon)
x = FlowElem_xcc;     y = FlowElem_ycc;    
xq = m(:,1);          yq = m(:,2);  

%Carregando o WaterLevel
    s1=nc_varget('*_map.nc','s1',...
